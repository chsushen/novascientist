...
class TopicProfileExtractor:
    ...
    def profile_topic(
        ...
        if any(k in t_lower for k in ["rag", "retrieval-augmented", "retrieval augmented", "factual consistency", "factuality", "hallucination", "question answering"]) or (dom_lower == "nlp" and any(k in t_lower for k in ["retrieval", "qa", "fact", "rag"])):
            ...
            paradigm = ResearchParadigm.SYSTEMS_OPTIMIZATION if "optimization" in t_lower or "communication" in t_lower or "latency" in t_lower else ResearchParadigm.THEORETICAL_ALGORITHMIC
            metrics = ["Global Model Accuracy (%)", "Client Drift Divergence (L2)", "Communication Rounds to Convergence", "Privacy Budget Epsilon (ε)"]
            primary_m = "Global Model Accuracy (%)"
            baselines = ["FedAvg (McMahan et al.)", "FedProx (Li et al.)", "SCAFFOLD (Karimireddy et al.)", "Local Independent Training"]
            method_families = ["Stochastic Controlled Averaging", "Adaptive Client Clustering", "Proximal Regularized Aggregation"]
            math_objs = ["Client Loss Functions f_i(w)", "Stochastic Gradient Variance σ_k^2", "Consensus Error ||w_t - w*||^2", "Privacy Budget (ε, δ)"]
            stat_tests = ["Paired Student's t-test across Seeds", "Wilcoxon Signed-Rank Test", "DerSimonian-Laird Meta-Analysis"]
            figs = ["Convergence Trajectories across Communication Rounds", "Client Heterogeneity Drift Heatmap", "Communication vs Accuracy Pareto Frontier", "Ablation over Non-IID Dirichlet Alpha"]
            requires_theorem = True

        elif dom_lower == "vision" or any(k in t_lower for k in ["medical image", "segmentation", "diffusion prior", "vision transformer", "vit", "resnet", "unet", "pixel", "convolution", "spatial attention"]):
            task = TaskType.OBJECT_DETECTION_SEGMENTATION if any(k in t_lower for k in ["segment", "lesion", "tumor", "organ", "image", "vision"]) else TaskType.CLASSIFICATION
            ...
            metrics = ["Dice Similarity Coefficient (DSC %)", "Intersection over Union (IoU %)", "Area Under ROC Curve (AUROC %)", "Expected Calibration Error (ECE %)"]
            primary_m = "Dice Similarity Coefficient (DSC %)"
            baselines = ["3D nnU-Net Standard Pipeline", "Vision Transformer (ViT)", "ResNet-50 Feature Backbone", "Dense V-Net Baseline"]
            method_families = ["Uncertainty-Guided Attention Refinement", "Boundary-Preserving Topological Loss", "Cross-Domain Contrastive Calibration"]
            math_objs = ["Generalized Dice Loss L_Dice", "Hausdorff Distance Sup_{x} Inf_{y} ||x - y||", "Calibration Reliability Curve ECE", "Inter-Observer Fleiss' Kappa"]
            stat_tests = ["Non-Parametric Wilcoxon Signed-Rank Test", "Delong Test for AUROC Differences", "DerSimonian-Laird Random-Effects Meta-Analysis"]
            ...
            paradigm = ResearchParadigm.EMPIRICAL_BENCHMARK if "benchmark" in t_lower else ResearchParadigm.METHODOLOGICAL_COMPARISON
            metrics = ["Mean Absolute Error (MAE)", "Root Mean Squared Error (RMSE)", "Continuous Ranked Probability Score (CRPS)", "Mean Absolute Percentage Error (MAPE %)"]
            primary_m = "Mean Absolute Error (MAE)"
            baselines = ["AutoARIMA / Classical VAR", "DeepAR Recurrent Network", "PatchTST Channel-Independent Transformer", "DLinear Decomposition Baseline"]
            method_families = ["Multi-Scale Patch Tokenization", "Frequency-Domain Fourier Operators", "Dynamic Lag-Attention Networks"]
            math_objs = ["Temporal Auto-Covariance Matrix Γ(τ)", "Multivariate State Transition Matrix A", "Spectral Density S(ω)", "Forecast Horizon Forecast Error e_{t+h}"]
            stat_tests = ["Diebold-Mariano Forecast Accuracy Test", "Paired t-test across Horizon Steps", "DerSimonian-Laird Random-Effects Meta-Analysis"]
            figs = ["Multi-Horizon Forecast Trajectories with 95% Confidence Intervals", "Residual Error Distribution Across Timesteps", "Horizon-Wise MAE Degradation Curves", "Patch Length Sensitivity Heatmap"]
            requires_theorem = False

        elif dom_lower == "nlp" or any(k in t_lower for k in ["nlp", "language", "transformer", "llm", "attention", "token", "prompt", "peft", "lora", "retrieval", "rag"]):
            task = TaskType.LANGUAGE_MODELING if any(k in t_lower for k in ["generat", "causal", "gpt", "pretrain", "causal language"]) else TaskType.CLASSIFICATION
            modality = DataModality.NATURAL_LANGUAGE_TEXT
            domain_name = "nlp"
            ...
            metrics = ["Relative L2 Spectral Error (%)", "Conservation Law Residual Loss", "Max Absolute Pointwise Error", "Inference Speedup Factor over Solver (x)"]
            primary_m = "Relative L2 Spectral Error (%)"
            baselines = ["Fourier Neural Operator (FNO-2D)", "DeepONet Dual-Branch Operator", "Physics-Informed Neural Network (PINN)", "Classical High-Order RK4 Solver"]
            method_families = ["Hamiltonian-Conserving Neural Operators", "Adaptive Wavelet Discretization", "Physics-Constrained Residual Networks"]
            math_objs = ["Differential Operator Residual N[u] - f = 0", "Energy Conservation Invariant H(q, p)", "Sobolev Norm ||u||_{H^s}", "Spectral Truncation Error Bound"]
            stat_tests = ["Paired Spectral Norm Difference Test", "DerSimonian-Laird Random-Effects Meta-Analysis"]
            figs = ["2D Solution Field Contour & Pointwise Error Maps", "Conservation Invariant Trajectory over Time", "Spectral Energy Spectrum vs Wavenumber", "PDE Residual Convergence Profile"]
            requires_theorem = True

        else:
            task = TaskType.CLASSIFICATION if "classif" in t_lower else TaskType.REGRESSION
            modality = DataModality.TABULAR_HETEROGENEOUS if any(k in t_lower for k in ["tabular", "credit", "financial", "clinical"]) else DataModality.IMAGE_VOLUMETRIC
            ...
            metrics = ["Classification Accuracy / Task Score (%)", "F1 Macro Score (%)", "Model Memory Footprint (MB)", "Inference Latency (ms)"]
            primary_m = "Classification Accuracy / Task Score (%)"
            baselines = ["Standard Full-Precision Baseline", "Regularized Competitive Model", "Lightweight Pruned Architecture"]
            method_families = ["Adaptive Representation Learning", "Regularized Stochastic Gradient Formulation", "Dynamic Feature Alignment"]
            math_objs = ["Empirical Risk Minimization R(θ)", "Generalization Bound E[R(θ) - R_emp(θ)]", "Loss Lipschitz Constant L", "Hessian Eigenvalue Distribution"]
            stat_tests = ["Paired Student's t-test", "DerSimonian-Laird Random-Effects Meta-Analysis"]
