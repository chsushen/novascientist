...
class MethodologySpec:
    ...
    hypothesis_evaluations: list[HypothesisEvaluationResult] = field(
...
class MethodologyAgent:
    ...
    def evaluate_hypotheses(
        ...
        meta_dict = (
            metrics_dict.get("meta_analysis")
        ...
        if not dense_key:
            for k in methods:
                if (
                    "dense" in k.lower()
                    or "baseline" in k.lower()
                    or "base_01" in k.lower()
                ):
                    dense_key = k
                    break
        ...
        for idx, hyp_text in enumerate(methodology.hypotheses):
            ...
            if any(
                v in h_lower
                for v in ["variance", "stability", "cross-seed", "std", "dispersion"]
            ) or ("stable" in h_lower and "split" in h_lower):
                threshold = 1.00
                prop_accs = _extract_values(
                    prop_runs, "final_accuracy"
                ) or _extract_values(prop_runs, "accuracy")
                ...
                if len(prop_accs) >= 2:
                    ...
                    chi2_stat = (
                        df * ((obs_std / threshold) ** 2) if threshold > 0 else 0.0
                    )
                    ...
                    ci_lo = (
                        math.sqrt(df * (obs_std**2) / chi2_hi) if chi2_hi > 0 else 0.0
                    )
                    ci_hi = (
                        math.sqrt(df * (obs_std**2) / chi2_lo)
                        if chi2_lo > 0
                        else float("inf")
                    )

                    exp_ids = [
                        f"{prop_key}_seed_{idx}" for idx in range(len(scaled_accs))
                    ]
                    ...
                    and "std_accuracy" in prop_data
                    and prop_data["std_accuracy"] is not None
                ):
                    ...
                    evaluations.append(
                        HypothesisEvaluation(
                            ...
                            decision=decision,
                            rationale=f"Observed aggregate variance {obs_std:.4f}% satisfies threshold (<= {threshold:.2f}%)."
                            if decision == HypothesisStatus.SUPPORTED
                            else f"Observed variance {obs_std:.4f}% exceeds {threshold:.2f}%.",
                        )
                    )
                else:
                    evaluations.append(
            ...
            # Case 2: Meta-Analytic Synthesis / Random-Effects Hypothesis
                ...
                if (
                    not meta_dict
                    or "z_statistic" not in meta_dict
                    or "p_value_z" not in meta_dict
                ):
                    evaluations.append(
                        HypothesisEvaluation(
                            hypothesis_id=hyp_id,
                ...
                ci = (
                    (round(float(ci_lo), 4), round(float(ci_hi), 4))
            ...
            # Case 3: Memory Footprint / Compression Hypothesis
                ...
                if len(p_mems) >= 2 and len(d_mems) >= 2 and len(p_mems) == len(d_mems):
                    reductions = [
                        ((d - p) / d * 100.0) for p, d in zip(p_mems, d_mems) if d > 0
                    ]
                    if not reductions:
                        evaluations.append(
                            HypothesisEvaluation(
                    ...
                    cohen_d, _ = StatisticalCriticAgent._compute_cohens_d(
                        d_mems, p_mems
                    )
                    summary = StatisticalCriticAgent._compute_metric_summary(
                        "mem_reduction", reductions
                    )
                    ci = (
                        (summary.ci_95_lower, summary.ci_95_upper)
                        if summary.ci_95_lower is not None
                        else None
                    )
                    exp_ids = [
                        f"{prop_key}_seed_{idx}" for idx in range(len(reductions))
                    ]
                    ...
                    if d_mean_mem > 0:
                        ...
                        evaluations.append(
                            HypothesisEvaluation(
                                ...
                                decision=decision,
                                rationale=f"Observed aggregate memory reduction of {obs_red:.2f}% satisfies threshold (>= {threshold:.1f}%)."
                                if decision == HypothesisStatus.SUPPORTED
                                else f"Observed memory reduction of {obs_red:.2f}% is below {threshold:.1f}%.",
                            )
                        )
                    else:
                        evaluations.append(
                    ...
                    if p_mean_lat > 0:
                        ...
                        evaluations.append(
                            HypothesisEvaluation(
                                ...
                                decision=decision,
                                rationale=f"Observed aggregate speedup of {obs_speedup:.2f}x satisfies threshold (>= {threshold:.1f}x)."
                                if decision == HypothesisStatus.SUPPORTED
                                else f"Observed speedup of {obs_speedup:.2f}x is below {threshold:.1f}x.",
                            )
                        )
                    else:
                        evaluations.append(
            ...
            # Case 5: Primary Performance / Accuracy / Generalization
                ...
                threshold = 0.0
                p_accs = _extract_values(
                    prop_runs, "final_accuracy"
                ) or _extract_values(prop_runs, "accuracy")
                d_accs = _extract_values(
                    dense_runs, "final_accuracy"
                ) or _extract_values(dense_runs, "accuracy")
                ...
                if len(p_accs) >= 2 and len(d_accs) >= 2 and len(p_accs) == len(d_accs):
                    ...
                    pairwise_test = StatisticalCriticAgent._conduct_pairwise_test(
                        ...
                        proposed_name=prop_data.get("name", prop_key or "proposed")
                        if isinstance(prop_data, dict)
                        else getattr(prop_data, "name", "proposed"),
                        baseline_id=dense_key or "baseline",
                        baseline_name=dense_data.get("name", dense_key or "baseline")
                        if isinstance(dense_data, dict)
                        else getattr(dense_data, "name", "baseline"),
                    ...
                    if p_val is None:
                        ...
                        "significant" in h_lower or "p <" in h_lower or "p<" in h_lower
                    ):
                    ...
                    evaluations.append(
                        HypothesisEvaluation(
                            ...
                            decision=decision,
                            rationale=f"Observed aggregate accuracy delta of {obs_delta:+.2f}% satisfies target (>= {threshold:.2f}%)."
                            if decision == HypothesisStatus.SUPPORTED
                            else f"Observed accuracy delta of {obs_delta:+.2f}% failed target ({threshold:.2f}%).",
                        )
                    )
                else:
                    evaluations.append(
    ...
    def synthesize_methodology(
        ...
        baseline_suite: BaselineComparisonSuite | None = None,
    ) -> MethodologySpec:
        """Generate a fully structured, topic-adaptive methodology specification."""
        m_hash = hashlib.sha256(
            (plan.plan_id + plan.topic_title).encode("utf-8")
        ).hexdigest()[:8]
        ...
        profile = topic_profile or TopicProfileExtractor.extract(
            ...
            if hasattr(plan.domain, "value")
            else str(plan.domain),
        )

        is_quant_topic = any(
            k in plan.topic_title.lower()
            for k in [
                "quantiz",
                "discretiz",
                "sub-linear memory",
        ...
        hypotheses = [
            ...
            "H3: The proposed architecture achieves significant positive effect size under DerSimonian-Laird random-effects meta-analysis (Z >= 1.96, p < 0.05).",
        ]
