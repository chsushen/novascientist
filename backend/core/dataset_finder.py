...
class DatasetFinder:
    ...
    def discover_candidates(
        cls,
        topic: str,
        domain: ComputationalDomain | str | None = None,
        limit: int = 3,
        ...
        if best_score <= 3.0 and (
            target_domain in {ComputationalDomain.APPLIED_ML, None}
            or not top_candidates
        ):
            words = [
                w.capitalize()
            ...
            ds_name = (
                f"{' '.join(words[:3])} Canonical Benchmark Dataset"
                if words
                else "Domain-Specific Evaluation Benchmark"
            )
    ...
    @classmethod
    def find_dataset_by_name(cls, name: str) -> DatasetMetadata | None:
        ...
        for ds in cls.DATASET_REGISTRY:
            if (
                ds.name.lower() == name_clean
                or name_clean in ds.name.lower()
                or ds.name.lower() in name_clean
            ):
    ...
    @classmethod
    def discover(cls, topic: str, domain: ComputationalDomain | str) -> DatasetMetadata:
