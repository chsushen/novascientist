...
class StatisticalAuditor:
    ...
    def audit_experiment_package(
        ...
        if len(methods) < 2:
            issues.append(
                "Comparative Baseline Violation: At least 2 methods (baseline and proposed) must be evaluated."
            )
        ...
        for m_id, m_data in methods.items():
            ...
            if std_acc == 0.0:
                issues.append(
                    f"Zero-Variance Warning: Method '{name}' exhibits 0.0 variance across seeds, indicating potential non-stochastic duplication."
                )
        ...
        if i_squared > 80.0:
            issues.append(
                f"Excessive Heterogeneity Warning: DerSimonian-Laird I² = {i_squared:.1f}% indicates severe inconsistency across evaluation folds."
            )
...
class RhetoricLinter:
    ...
    OVERCLAIM_PATTERNS = [
        ...
        (
            r"\b(universal solution|universal paradigm)\b",
            "broadly applicable framework",
        ),
        (
            r"\b(unbeatable SOTA|the definitive SOTA|SOTA)\b",
        ...
        (
            r"\b(vastly superior to all)\b",
            "empirically outperforming evaluated baselines",
        ),
    ]

    @classmethod
    def lint_and_refactor(
        cls, manuscript_text: str
        ...
        for pattern, replacement in cls.OVERCLAIM_PATTERNS:
            ...
            if matches:
                ...
                modified_text = re.sub(
                    pattern, replacement, modified_text, flags=re.IGNORECASE
                )
...
class AdversarialReviewerSwarm:
    ...
    def review_manuscript(
        cls, metrics_data: dict[str, Any], raw_latex: str
    ) -> tuple[str, ReviewerAuditReport]:
        """Perform full adversarial review on metrics and LaTeX text."""
        stat_passed, stat_issues = StatisticalAuditor.audit_experiment_package(
            metrics_data
        )
        ...
        if not stat_passed:
            recommendations.append(
                "Increase seed budget to k >= 5 and re-run baseline simulations."
            )
        if rhetoric_mods:
            recommendations.append(
                f"Refactored {len(rhetoric_mods)} unhedged assertions into qualified scientific language."
            )
...
class ReviewerSwarm:
    ...
    def conduct_audit(self) -> ReviewerAuditReport:
        cleaned, report = AdversarialReviewerSwarm.review_manuscript(
            self.metrics_dict, self.latex_content
        )
        return report
