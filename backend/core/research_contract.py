...
from backend.core.topic_profile import (
    DataModality,
    ResearchParadigm,
    TaskType,
...
class HypothesisEvaluation:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            if isinstance(self.decision, HypothesisStatus)
...
class StatisticalPlan:
    ...
    distribution_assumption: (
        str  # 'normal' | 'non_normal' | 'ordinal' | 'heterogeneous'
    )
    ...
    def to_dict(self) -> dict[str, Any]:
        d = asdict(self)
        d["test_selection"] = (
            self.test_selection.value
            if isinstance(self.test_selection, StatisticalAnalysisType)
            else str(self.test_selection)
        )
        return d
...
class EvidenceDecisionRecord:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if isinstance(self.selected_value, Enum):
            ...
            d["selected_value"] = [
                v.value if isinstance(v, Enum) else v for v in self.selected_value
            ]
        return d
...
class ScientificResearchContract:
    ...
    methodology_spec: Any | None = None
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        if getattr(self, "is_frozen", False) and name not in (
            "is_frozen",
    ...
    def selected_experiment_methods(self) -> list[str]:
        ...
        if self.selected_method:
            slug = re.sub(r"[^a-zA-Z0-9_]", "_", self.selected_method.lower().strip())[
                :32
            ].strip("_")
            res.append(slug)
        return res
    ...
    def validate_downstream_state(
        self,
        experiment_records: list[Any] | None = None,
        hypothesis_evaluations: list[HypothesisEvaluation] | None = None,
        ...
        if evals:
            ...
            for h_eval in evals:
                ...
                if any(
                    ...
                    if not any(
                        v in m_name
                        for v in ["variance", "standard deviation", "std", "dispersion"]
                    ):
                        violations.append(
                ...
                if h_eval.decision == HypothesisStatus.SUPPORTED:
                    ...
                    if h_eval.p_value is None and h_eval.statistical_test not in (
                        "constant_difference",
                        "relative_reduction_ratio",
                    ):
                        violations.append(
                            f"SCIENTIFIC INTEGRITY VIOLATION: Hypothesis '{h_eval.hypothesis_id}' is marked SUPPORTED without a valid computed p-value."
                        )
                    if (
                        ("significant" in stmt or "p <" in stmt or "p<" in stmt)
                        and h_eval.p_value is not None
                        and h_eval.p_value >= 0.05
                    ):
                        violations.append(
                ...
                if known_exp_ids and h_eval.experiment_ids:
                    for eid in h_eval.experiment_ids:
                        if (
                            not eid.startswith("meta_analysis")
                            and eid not in known_exp_ids
                            and not any(k in eid or eid in k for k in known_exp_ids)
                        ):
                            violations.append(
        ...
        if experiment_records:
            contracted_methods = set(
                self.selected_baselines
                + [self.selected_method]
                + self.selected_experiment_methods
            )
            for er in experiment_records:
                ...
                matches = (
                    ...
                    or any(
                        c.lower() in m_name.lower() or m_name.lower() in c.lower()
                        for c in contracted_methods
                    )
                )
                if not matches:
                    violations.append(
                        f"Uncontracted experiment method '{m_name}' ({m_id}) executed in active run."
                    )

                # Check for legacy forbidden method contamination
                contract_has_quant = any(
                    "quantiz" in k.lower() or "int8" in k.lower()
                    for k in [self.research_question, self.selected_method]
                    + self.selected_baselines
                )
                if not contract_has_quant and (
                    "int8" in m_name.lower() or "post_int8" in m_id.lower()
                ):
                    violations.append(
                        f"Legacy quantization method '{m_name}' contaminated non-quantization contract."
                    )

                contract_has_gnn = any(
                    "gnn" in k.lower() or "graph" in k.lower()
                    for k in [self.research_question, self.selected_method]
                    + self.selected_baselines
                )
                if not contract_has_gnn and (
                    "sparse_gnn" in m_id.lower()
                    or "gnn" in m_name.lower()
                    and "rag" in self.research_question.lower()
                ):
                    violations.append(
                        f"Legacy sparse GNN method '{m_name}' contaminated non-graph contract."
                    )
        ...
        if figures is not None:
            if not self.figure_requirements and len(figures) > 0:
                violations.append(
                    f"Generated {len(figures)} figures when contract specifies zero figures."
                )
        ...
        if pdf_path and os.path.exists(pdf_path):
            try:
                ...
                is_journal = (
                    "8_12" in str(getattr(self, "research_question", ""))
                    or "appendix" in (latex_content or "").lower()
                )
                if is_journal:
                    if pdf_pages < 8 or pdf_pages > 12:
                        violations.append(
                            f"Physical PDF page count {pdf_pages} violates Extended Journal target (8–12 pages)."
                        )
                else:
                    if pdf_pages < 6 or pdf_pages > 8:
                        violations.append(
                            f"Physical PDF page count {pdf_pages} violates Standard Conference target (6–8 pages)."
                        )
            except Exception as e:
                violations.append(f"Failed to verify physical PDF page count: {e!s}")
        ...
        return {
            ...
            "hypothesis_integrity": "PASS"
            if not any("Hypothesis" in v for v in violations)
            else "FAIL",
            "experiment_integrity": "PASS"
            if not any("experiment" in v or "method" in v for v in violations)
            else "FAIL",
            "statistical_integrity": "PASS"
            if not any("statistical" in v or "meta-analysis" in v for v in violations)
            else "FAIL",
            "figure_integrity": "PASS"
            if not any("figure" in v for v in violations)
            else "FAIL",
            "provenance_integrity": "PASS"
            if not any("provenance" in v or "DAG" in v for v in violations)
            else "FAIL",
            "manuscript_integrity": "PASS"
            if not any("manuscript" in v or "theorem" in v for v in violations)
            else "FAIL",
            "pdf_integrity": "PASS"
            if not any("PDF" in v for v in violations)
            else "FAIL",
        }
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        d["dataset_decision"] = (
            self.dataset_decision.to_dict()
            if isinstance(self.dataset_decision, EvidenceDecisionRecord)
            else self.dataset_decision
        )
        d["method_decision"] = (
            self.method_decision.to_dict()
            if isinstance(self.method_decision, EvidenceDecisionRecord)
            else self.method_decision
        )
        d["baselines_decision"] = (
            self.baselines_decision.to_dict()
            if isinstance(self.baselines_decision, EvidenceDecisionRecord)
            else self.baselines_decision
        )
        d["metrics_decision"] = (
            self.metrics_decision.to_dict()
            if isinstance(self.metrics_decision, EvidenceDecisionRecord)
            else self.metrics_decision
        )
        d["experiments_decision"] = (
            self.experiments_decision.to_dict()
            if isinstance(self.experiments_decision, EvidenceDecisionRecord)
            else self.experiments_decision
        )
        d["math_decision"] = (
            self.math_decision.to_dict()
            if isinstance(self.math_decision, EvidenceDecisionRecord)
            else self.math_decision
        )
        d["statistics_decision"] = (
            self.statistics_decision.to_dict()
            if isinstance(self.statistics_decision, EvidenceDecisionRecord)
            else self.statistics_decision
        )
        d["figures_decision"] = (
            self.figures_decision.to_dict()
            if isinstance(self.figures_decision, EvidenceDecisionRecord)
            else self.figures_decision
        )
        d["manuscript_decision"] = (
            self.manuscript_decision.to_dict()
            if isinstance(self.manuscript_decision, EvidenceDecisionRecord)
            else self.manuscript_decision
        )
        d["research_gap"] = (
            self.research_gap.to_dict()
            if isinstance(self.research_gap, ResearchGapRecord)
            else self.research_gap
        )
...
class QuestionDecompositionEngine:
    ...
    @classmethod
    def decompose(
        cls, topic: str, context: str | None = None, domain: str | None = None
        ...
        is_signal_vibration = dom_lower in [
            "signal_processing",
            "industrial_iot",
            "vibration",
        ] or any(
        ...
        is_probabilistic_ts = any(
            k in t_lower
            for k in ["probabilistic", "uncertainty", "calibration", "quantile", "crps"]
        ) and any(k in t_lower for k in ["forecast", "time", "temporal"])
        is_ts = (
            any(
                k in t_lower
                for k in [
        ...
        if is_rag_qa:
            ...
            constraints = [
                "Parametric generation hallucination risk",
                "Context length window budget",
            ...
            evidence = [
                "Factual consistency vs retrieval depth curves",
                "Hallucination rate comparison across context densities",
            ]
            ...
            constraints = [
                "Severe industrial background noise",
                "Variable operating speeds",
                "Early incipient fault signatures",
            ]
            ...
            eval_proto = (
                "Multi-regime cross-validation across diverse bearing operational loads"
            )
            evidence = [
                "Time-frequency spectrogram activation",
                "AUROC curves across operational regimes",
            ]
            ...
            ind_vars = [
                "Graph neighborhood aggregation depth (K)",
            ...
            constraints = [
                "Severe class imbalance (< 1% minority)",
                "Structural edge sparsity",
                "Temporal graph drift",
            ]
            comp = (
                "Standard homogeneous GNNs (GCN, GAT) and imbalanced sampling baselines"
            )
            hyp_type = "Robust minority-class fraud detection under severe topological imbalance and drift"
            eval_proto = (
                "Temporal graph split with strict chronological test evaluation"
            )
            evidence = [
                "Precision-Recall frontiers",
                "Neighbor aggregation sensitivity across drift windows",
            ]
            ...
            ind_vars = [
                "Client heterogeneity (Dirichlet alpha)",
                "Local step budget (tau)",
                "Active client fraction",
            ]
            dep_vars = [
            ...
            constraints = [
                "Local data privacy (zero raw data transmission)",
                "Bandwidth-constrained communication",
            ]
            comp = "Federated Averaging (FedAvg), FedProx, and SCAFFOLD"
            hyp_type = (
                "Accelerated consensus with bounded client drift on non-IID partitions"
            )
            ...
            task = (
                "text_classification"
                if any(
                    k in t_lower for k in ["classif", "sentiment", "topic", "detect"]
                )
            ...
            ind_vars = [
                "Trainable parameter budget (r)",
                "Adapter rank / placement",
                "Domain shift gap",
            ]
            dep_vars = [
                "Macro F1 Score (%)",
            ...
            evidence = [
                "Macro-F1 score vs trainable parameter tradeoff",
                "Multi-seed classification convergence trajectories",
            ]
            ...
            ind_vars = [
                "Forecast horizon (H)",
                "Uncertainty quantile levels (alpha)",
            ...
            constraints = [
                "Temporal causality (no lookahead leakage)",
            ...
            evidence = [
                ...
                "CRPS horizon degradation curves",
            ]
            ...
            hyp_type = (
                "Predictive accuracy and stability under temporal distribution shift"
            )
            eval_proto = "Rolling-window multi-horizon cross-validation with out-of-distribution evaluation"
            evidence = [
                "Empirical horizon degradation curves",
                "Multi-dataset error distributions",
            ]
            ...
            constraints = [
                "High computational resolution constraints",
                "Boundary precision requirements",
            ]
            comp = "Standard 3D U-Net and Vision Transformer baselines"
            hyp_type = (
                "High boundary precision and calibration under acquisition domain shift"
            )
            ...
            evidence = [
                "Benchmark comparative metrics across seeds",
...
class ResearchContractBuilder:
    ...
    def build_contract(
        ...
        experimental_design: dict[str, Any] | None = None,
    ) -> ScientificResearchContract:
        """Derive a complete, evidence-ranked ScientificResearchContract."""
        decomp = QuestionDecompositionEngine.decompose(
            topic, context=context, domain=profile.domain
        )
        c_hash = hashlib.sha256(
            f"{topic}_{decomp.domain}_{decomp.task}".encode()
        ).hexdigest()[:10]
        ...
        discovered_ds = DatasetFinder.discover(
            topic, profile.domain if profile else None
        )
        default_ds_name = (
            discovered_ds.name if discovered_ds else "Canonical Benchmark Dataset"
        )
        dataset_candidates = (
            profile.candidate_datasets if profile and profile.candidate_datasets else []
        ) or [default_ds_name, "Standard Reference Corpus"]
        ...
        dataset_dec = EvidenceDecisionRecord(
            ...
            supporting_sources=[
                getattr(
                    literature_report,
                    "domain_overview",
                    "Literature Benchmark Standard",
                )
            ],
            supporting_passages=[
                f"Standard benchmark dataset widely adopted for {decomp.task} in {decomp.subdomain}."
            ],
        ...
        method_candidates = profile.candidate_method_families or [
            "Adaptive Representation Architecture",
            "Dynamic Attention Network",
        ]
        selected_method = (
            profile.model_full_name_suggestion or f"Adaptive {decomp.subdomain} Model"
        )
        method_dec = EvidenceDecisionRecord(
            ...
            supporting_sources=[],
            supporting_passages=[
                f"Directly targets the identified gap in {decomp.subdomain}."
            ],
        ...
        baseline_candidates = profile.candidate_baselines or [
        ...
        baselines_dec = EvidenceDecisionRecord(
            ...
            supporting_sources=[
                ...
                if getattr(b, "doi", None)
            ],
            supporting_passages=[
                getattr(b, "selection_rationale", "")
                for b in getattr(literature_report, "recommended_baselines", [])
            ],
        ...
        metric_pool = []
        if (
            decomp.task == "question_answering"
            or "rag" in decomp.subdomain.lower()
            or "factual" in decomp.subdomain.lower()
        ):
            metric_pool = [
                "Factual Consistency Score (%)",
                "Exact Match (EM %)",
            ...
            decomp.domain == "signal_processing"
            or "vibration" in decomp.subdomain.lower()
        ):
            metric_pool = [
                "Fault Detection F1-Score (%)",
                "Area Under ROC Curve (AUROC %)",
        ...
        elif decomp.task == "text_classification" or (
            "nlp" in decomp.domain and "classification" in decomp.task
        ):
            metric_pool = [
                "Macro F1 Score (%)",
                "Top-1 Accuracy (%)",
            ...
            metric_pool = profile.candidate_metrics or [
                "Primary Task Score (%)",
                "Inference Latency (ms)",
            ]
            ...
            secondary_metrics = (
                metric_pool[2:4]
                if len(metric_pool) > 2
                else ["Resource Footprint (MB)"]
            )
        ...
        req_exp = [
            f"Comparative Multi-Seed Benchmark across {', '.join(selected_baselines[:2])}"
        ]
        if "shift" in topic.lower() or "drift" in topic.lower():
            req_exp.append(
                f"Distribution Shift & Degradation Analysis across {decomp.independent_variables[0]}"
            )
        elif (
            "efficiency" in topic.lower()
            or "parameter" in topic.lower()
            or "peft" in topic.lower()
        ):
            req_exp.append(
                "Parameter Budget Scaling & Trainable Parameter Efficiency Analysis"
            )
        elif (
            "uncertainty" in topic.lower()
            or "calibration" in topic.lower()
            or "probabilistic" in topic.lower()
        ):
            req_exp.append(
                "Uncertainty Calibration & Quantile Reliability Audit across Horizons"
            )
        elif "imbalance" in topic.lower() or "fraud" in topic.lower():
            req_exp.append(
                "Class Imbalance Ratio & Minority Precision-Recall Sensitivity Grid"
            )
        elif (
            "rag" in topic.lower()
            or "retrieval" in topic.lower()
            or "question answering" in topic.lower()
        ):
            req_exp.append(
                "Retrieval Depth (Top-k) & Context Density Sensitivity Analysis"
            )
        else:
        ...
        exp_dec = EvidenceDecisionRecord(
            ...
            supporting_sources=[],
            supporting_passages=[
                f"Designed to test the core hypotheses: {hypotheses[0]}"
            ],
        ...
        has_formal_convergence_claim = any(
            k in topic.lower()
            for k in [
        ...
        has_optimization_formulation = any(
            k in topic.lower()
            for k in [
                "parameter-efficient",
                "peft",
        ...
        is_multicenter = exp_design.get("is_multicenter", False) or (
            "meta" in topic.lower()
            or "multicohort" in topic.lower()
            or "multicenter" in topic.lower()
        )
        num_groups = exp_design.get("num_groups", len(selected_baselines) + 1)
        is_single_run = (
            exp_design.get("is_single_run", False)
            or num_seeds <= 1
            or "observational" in topic.lower()
            or "case study" in topic.lower()
        )
        ...
        if is_single_run or num_seeds <= 1:
            ...
            stat_conf = 0.95
        elif distribution_type in ("non_normal", "ordinal", "rank") or any(
            k in topic.lower()
            for k in ["non-parametric", "ordinal", "rank", "wilcoxon", "heavy-tail"]
        ):
        ...
        is_convergence_study = any(
            ...
            for k in [
                ...
                "loss convergence",
                "non-iid coordination",
            ]
        ) and not any(
            k in topic.lower()
            for k in ["zero-shot", "post-hoc", "inference-only", "calibration"]
        )
        if is_convergence_study:
            fig_candidates.append("Multi-Seed Convergence & Variance Band")

        # Task-specific communication figures
        if decomp.task == "timeseries_forecasting" and any(
            k in topic.lower() for k in ["shift", "horizon", "long-horizon", "forecast"]
        ):
            fig_candidates.append("Forecast Trajectories vs Ground Truth")
            fig_candidates.append("Horizon-Wise Error Degradation Curve")
        elif (
            decomp.task == "probabilistic_forecasting"
            or "calibration" in topic.lower()
            or "uncertainty" in topic.lower()
        ):
        ...
        elif decomp.task == "text_classification" and (
            "peft" in topic.lower() or "parameter" in topic.lower()
        ):
            fig_candidates.append(
                "Macro-F1 vs Trainable Parameter Footprint Pareto Frontier"
            )
        ...
        elif decomp.task == "graph_reasoning" and (
            "imbalance" in topic.lower() or "fraud" in topic.lower()
        ):
            fig_candidates.append(
        ...
        if (
            not fig_candidates
            and profile.research_paradigm != ResearchParadigm.THEORETICAL_ALGORITHMIC
            and not any(
                k in topic.lower()
                for k in ["zero figure", "purely tabular", "meta-review"]
            )
        ):
        ...
        if literature_report and getattr(literature_report, "candidate_gaps", None):
            ...
            has_supporting_sources = bool(
                getattr(gap_item, "supporting_source_ids", None)
            )
            gap_status = (
                "EVIDENCE_SUPPORTED"
                if has_supporting_sources
            ...
            gap_rec = ResearchGapRecord(
                ...
                source_ids=gap_item.supporting_source_ids
                if has_supporting_sources
                else [],
                supporting_passage=gap_item.supporting_passages[0]
                if getattr(gap_item, "supporting_passages", None)
                else "",
                why_current_methods_fail=f"Current methods exhibit performance degradation under {decomp.independent_variables[0]}."
                if has_supporting_sources
                else "Limitation not explicitly verified in retrieved literature.",
                why_existing_work_does_not_resolve_it="Prior literature focuses primarily on stationary evaluation setups."
                if has_supporting_sources
                else "Candidate hypothesis requiring empirical verification.",
        ...
        effect_metric = (
            "Cohen's d"
            if stat_req == StatisticalAnalysisType.PAIRED_T_TEST
            else (
                "Rank-Biserial"
                if stat_req == StatisticalAnalysisType.WILCOXON_SIGNED_RANK
                else (
                    ...
                    if stat_req == StatisticalAnalysisType.RANDOM_EFFECTS_META_ANALYSIS
                        ...
                        "Bootstrap 95% CI"
                        if stat_req
                        == StatisticalAnalysisType.BOOTSTRAP_CONFIDENCE_INTERVAL
                        else "Empirical Mean Difference"
                    )
                )
            )
        )
        ...
        lit_records = []
        if literature_report and getattr(
            literature_report, "recommended_baselines", None
        ):
            for b in literature_report.recommended_baselines:
                lit_records.append(
                    LiteratureDecisionRecord(
                        ...
                        retrieved_passage=getattr(
                            b,
                            "selection_rationale",
                            "Canonical baseline in task domain.",
                        ),
...
def validate_downstream_against_contract(
    ...
    if isinstance(artifact, dict) and (
        "methods" in artifact or "metrics" in artifact or "meta_analysis" in artifact
    ):
    ...
    # 2. Inspect Figures List
        ...
        and len(artifact) > 0
        and (hasattr(artifact[0], "figure_type") or isinstance(artifact[0], dict))
    ):
        req_types = [r.lower() for r in contract.figure_requirements]
        for fig in artifact:
            fig_title = getattr(
                fig, "title", fig.get("title", "") if isinstance(fig, dict) else ""
            )
            fig_type = getattr(
                fig,
                "figure_type",
                fig.get("figure_type", "") if isinstance(fig, dict) else "",
            )
            ...
            if not req_types and fig_type_str not in [
                "system_architecture",
                "architecture",
            ]:
                raise ScientificContractViolationError(
    ...
    # 3. Inspect LaTeX Manuscript String
        ...
        "\\documentclass" in artifact or "\\section{" in artifact
    ):
        tex = artifact
        ...
        if (
            ...
            != StatisticalAnalysisType.RANDOM_EFFECTS_META_ANALYSIS
        ):
        ...
        if contract.selected_dataset and len(contract.selected_dataset) > 4:
            ...
            if (
                "METR-LA" in tex
                and "metr-la" not in contract.selected_dataset.lower()
                and "metr" not in contract.research_question.lower()
            ):
                raise ScientificContractViolationError(
...
def generate_contract_consistency_report(
    ...
    if not contract_has_hardware:
        ...
        for term in forbidden:
            if term in latex_content:
                unauthorized_methodology.append(
                    f"Found uncontracted hardware term '{term}'."
                )
    ...
    if contract.mathematical_requirement in (
        ...
        if "\\begin{theorem}" in latex_content or "\\begin{lemma}" in latex_content:
            unauthorized_mathematics.append(
                f"Found formal theorem/lemma in manuscript despite mathematical requirement '{contract.mathematical_requirement.value}'."
            )
    ...
    if (
        ...
        if (
            "DerSimonian-Laird" in latex_content
            or "DerSimonian--Laird" in latex_content
        ):
            unauthorized_statistics.append(
                "Found DerSimonian-Laird meta-analysis in manuscript despite non-meta-analysis requirement."
            )

    # 5. Figures check
    if not contract.figure_requirements and figures:
        unauthorized_figures.append(
            f"Found {len(figures)} figures generated when contract specifies zero figures."
        )
