...
class ScientificReviewerAgent:
    ...
    def review(
        ...
        methods_dict = metrics_dict.get("methods", {}) if isinstance(metrics_dict, dict) else {}
        proposed_found = False
        if any("proposed" in str(k).lower() or "mb_qgt" in str(k).lower() for k in methods_dict.keys()):
            proposed_found = True
        if "proposed" in latex_text.lower() or "mb-qgt" in latex_text.lower() or "architecture" in latex_text.lower():
            proposed_found = True
    ...
    def apply_targeted_revisions(
        ...
        for pat, replacement in ScientificReviewerAgent.UNHEDGED_PATTERNS:
            if re.search(pat, updated_latex, re.IGNORECASE):
                updated_latex = re.sub(pat, replacement, updated_latex, flags=re.IGNORECASE)
                actions_applied.append(f"Refactored unhedged language matching '{pat}' -> '{replacement}'.")
        ...
        if has_limitations_finding and "limitation" not in updated_latex.lower():
            ...
            if "\\section{Conclusion" in updated_latex:
                updated_latex = updated_latex.replace("\\section{Conclusion", limitations_block + "\n\\section{Conclusion", 1)
            elif "\\section{Discussion" in updated_latex:
                updated_latex = updated_latex.replace("\\section{Discussion", limitations_block + "\n\\section{Discussion", 1)
            elif "\\end{document}" in updated_latex:
                updated_latex = updated_latex.replace("\\end{document}", limitations_block + "\n\\end{document}", 1)
            else:
                updated_latex += f"\n{limitations_block}\n"
            actions_applied.append("Injected dedicated Limitations and Boundary Conditions subsection.")

        # 3. Address Reproducibility Findings
        has_repro_finding = any(f.category == "reproducibility" for f in findings)
        if has_repro_finding and "reproducibility" not in updated_latex.lower() and "seed" not in updated_latex.lower():
            ...
            if "\\section{Experiments" in updated_latex:
                updated_latex = updated_latex.replace("\\section{Experiments", "\\section{Experiments}\n" + repro_block, 1)
            elif "\\section{Methodology" in updated_latex:
                updated_latex = updated_latex.replace("\\section{Methodology", "\\section{Methodology}\n" + repro_block, 1)
            else:
                updated_latex += f"\n{repro_block}\n"
            actions_applied.append("Injected formal Reproducibility and Determinism specifications.")
        ...
        if has_novelty_finding and "proposed architecture" not in updated_latex.lower():
            ...
            if "\\section{Introduction}" in updated_latex:
                updated_latex = updated_latex.replace("\\section{Introduction}", "\\section{Introduction}\n" + novelty_block, 1)
            else:
    ...
    def run_revision_loop(
        ...
        for iter_num in range(1, self.MAX_REVIEW_ITERATIONS + 1):
            ...
            if report.passed:
                ...
                stopped_reason = f"All review criteria satisfied on Revision {iter_num}."
                break
