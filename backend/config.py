...
class NovaScientistConfig:
    ...
    debug: bool = os.getenv("NOVASCIENTIST_DEBUG", "false").lower() in ("true", "1", "yes")
    
    # Storage & Workspace
    data_dir: Path = field(
        default_factory=lambda: Path(os.getenv("NOVASCIENTIST_DATA_DIR", ".novascientist_data")).resolve()
    )
    ...
    llm_cost_limit_usd: float = float(os.getenv("NOVASCIENTIST_LLM_COST_LIMIT_USD", "10.0"))
    ...
    sandbox_max_memory_mb: int = int(os.getenv("NOVASCIENTIST_SANDBOX_MAX_MEMORY_MB", "1024"))
    demo_mode: bool = os.getenv("NOVASCIENTIST_DEMO_MODE", "false").lower() in ("true", "1", "yes")
