...
class LLMCostTracker:
    ...
    def __init__(self, cost_limit_usd: Optional[float] = None) -> None:
...
class LLMProvider(ABC):
    ...
    def __init__(self, cost_tracker: Optional[LLMCostTracker] = None) -> None:
...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gpt-4o", cost_tracker: Optional[LLMCostTracker] = None) -> None:
    ...
    async def generate(
        ...
        if not self.api_key:
            ...
            logger.warning("OpenAI API key not set; falling back to MockDeterministicProvider.")
            mock = MockDeterministicProvider(cost_tracker=self.cost_tracker)
            return await mock.generate(prompt, system_prompt, max_tokens, temperature, expected_json_keys)
...
class AnthropicProvider(LLMProvider):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "claude-3-5-sonnet-20241022", cost_tracker: Optional[LLMCostTracker] = None) -> None:
        super().__init__(cost_tracker)
    ...
    async def generate(
        ...
        if not self.api_key:
            logger.warning("Anthropic API key not set; falling back to MockDeterministicProvider.")
            mock = MockDeterministicProvider(cost_tracker=self.cost_tracker)
            return await mock.generate(prompt, system_prompt, max_tokens, temperature, expected_json_keys)
...
class GeminiProvider(LLMProvider):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gemini-1.5-pro", cost_tracker: Optional[LLMCostTracker] = None) -> None:
        super().__init__(cost_tracker)
    ...
    async def generate(
        ...
        if not self.api_key:
            logger.warning("Gemini API key not set; falling back to MockDeterministicProvider.")
            mock = MockDeterministicProvider(cost_tracker=self.cost_tracker)
            return await mock.generate(prompt, system_prompt, max_tokens, temperature, expected_json_keys)
...
class FallbackProvider(LLMProvider):
    ...
    async def generate(
        ...
        for p in providers:
            try:
                return await p.generate(prompt, system_prompt, max_tokens, temperature, expected_json_keys)
            except Exception as exc:
                logger.warning(f"Provider {type(p).__name__} failed: {exc}. Trying next fallback...")
                last_err = exc

        raise RuntimeError(f"All LLM providers in fallback chain failed. Last error: {last_err}")
